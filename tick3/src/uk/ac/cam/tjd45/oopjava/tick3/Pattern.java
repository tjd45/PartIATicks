...
public class Pattern {
	...
	public Pattern(String format) throws PatternFormatException {
		...
		try {
			width = Integer.parseInt(arrofString[2]); // error: not an integer
														// value
		} catch (NumberFormatException error) {
			throw new PatternFormatException(
					"Invalid pattern format: Could not interpret the width field as a number ('" + arrofString[2]
							+ "' given).");
		}
		try {
			height = Integer.parseInt(arrofString[3]); // error: not an integer
														// value
		} catch (NumberFormatException error) {
			throw new PatternFormatException(
					"Invalid pattern format: Could not interpret the height field as a number ('" + arrofString[3]
							+ "' given).");
		}
		try {
			startCol = Integer.parseInt(arrofString[4]); // error: not an
															// integer value
		} catch (NumberFormatException error) {
			throw new PatternFormatException(
					"Invalid pattern format: Could not interpret the startX field as a number ('" + arrofString[4]
							+ "' given).");
		}
		try {
			startRow = Integer.parseInt(arrofString[5]); // error: not an
															// integer value
		} catch (NumberFormatException error) {
			throw new PatternFormatException(
					"Invalid pattern format: Could not interpret the startX field as a number ('" + arrofString[5]
							+ "' given).");
		}
		cells = arrofString[6];
