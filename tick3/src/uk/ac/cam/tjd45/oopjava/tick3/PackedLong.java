...
public class PackedLong {
	 ...
	 * position counts from zero (representing the least significant bit) up to
	 * 63 (representing the most significant bit).
	 */
	public static boolean get(long packed, int position) {
		// set "check" to equal 1 if the "position" bit in "packed" is set to 1
		long check = 1L & (packed >> position);// TODO: complete this statement
	...
	public static long set(long packed, int position, boolean value) {
		if (value) {
			packed |= (1L << position);// TODO: complete this
